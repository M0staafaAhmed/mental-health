...
import { Brain, Frown, HelpCircle, Repeat, ShieldCheck, Zap, Sparkles } from 'lucide-react';
...
function getTestStyling(testName: string) {
    ...
    if (name.includes("adhd"))
        return {
            icon: Zap,
            bg: "bg-amber-50/80 border border-amber-100", // غيرتها لـ Amber تماشي طبيعة الـ الـ ADHD والـ Energy
            text: "text-amber-500",
...
export default function TestCard({ test }: { test: TestType }) {
    ...
    return (
        <div className="min-w-70 sm:min-w-72.5 bg-white/90 rounded-4xl p-6 border border-slate-100 flex flex-col gap-5 relative shadow-soft hover:shadow-soft-hover transition-all duration-300 hover:-translate-y-1 backdrop-blur-sm group">
            ...
            <div className={`w-12 h-12 rounded-2xl ${bg} flex items-center justify-center transition-transform duration-300 group-hover:scale-105 filter drop-shadow-sm`}>
